<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class Renewal extends Model
{
    use HasFactory;

    protected $fillable = [
        'user_id', 'registration_id', 'form_id', 'type', 'renewal_year', 'expires_at',
        'licence', 'status', 'renewal', 'inspection', 'inspection_year', 'payment', 'query', 'token', 'recommendation_status'
    ];

    public function user() {
        return $this->hasOne(User::class,'id', 'user_id');
    }

    public function hospital_pharmacy() {
        return $this->hasOne(HospitalRegistration::class,'id', 'form_id');
    }

    public function registration() {
        return $this->hasOne(Registration::class,'id', 'registration_id');
    }

    public function ppmv() {
        return $this->hasOne(PpmvLocationApplication::class,'id', 'form_id');
    }

    public function other_registration() {
        return $this->hasOne(OtherRegistration::class,'id', 'form_id');
    }
}
