...
@section('content')
@include('layouts.navbars.breadcrumb', ['page' => 'Facility Inspection Report', 'route' => 'licence-pending.index'])
<div class="row">
<div class="col-lg-12 col-md-12">
    <form id="approvedForm" class="w-100" method="POST" action="{{ route('licence-pending-approve-all') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <div class="row m-0">
                ...
                <div class="col-sm-12 col-md-6">
                    ...
                    <form method="GET" action="{{ route('licence-pending.index') }}">
                    @csrf
                        <label>Search:
            ...
            <table class="display table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @endif
                        @if($document->type == 'manufacturing_premises')
                        <td>Pharmaceutical Manufacturing Premises Registration Application</td>
                        @endif
                        ...
                        <td>
                            @if($document->type == 'hospital_pharmacy')
                            <a href="{{ route('licence-pending-hospital-show') }}?registration_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->type == 'ppmv')
                            <a href="{{ route('licence-pending-ppmv-show') }}?registration_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->type == 'community_pharmacy')
                            <a href="{{ route('licence-pending-community-show') }}?registration_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->type == 'distribution_premises')
                            <a href="{{ route('licence-pending-distribution-show') }}?registration_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->type == 'manufacturing_premises')
                            <a href="{{ route('licence-pending-manufacturing-show') }}?registration_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                        </td>
        ...
        <button onclick="approveSelected(event)" type="button" class="btn btn-primary mt-5">ISSUE LICENCE (SELECTED)</button>
    </div>
</div>
</form>
</div>
