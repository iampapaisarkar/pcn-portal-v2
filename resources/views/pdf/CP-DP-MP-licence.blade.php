...
<body style="background-image: url({{$background}}); width: 100%; height: 100%;">
    <div style="padding-top: 32px;padding-left: 70px;padding-right: 70px;">
        ...
        <div style="position: absolute; top: 250px;">
            ...
            @if($type == 'community_pharmacy_renewal')
            <div style="margin-bottom: 20px; font-size: 25px; text-align: center;  color: #bb1c25;"><span>was on <strong>{{strtoupper($data['updated_at']->format('jS'))}} DAY OF  {{strtoupper($data['updated_at']->format('M'))}}, {{$data['updated_at']->format('Y')}}</strong> duly registered for the purposeof mixing, compounding, preparing, counselling and selling ofdrugs, medicines and poisons to patients in accordance with theprovisions of the Pharmacists Council of Nigeria Act, Cap P17,LFN, 2004</span></div>
            @endif
            @if($type == 'distribution_premises_renewal')
            <div style="margin-bottom: 20px; font-size: 25px; text-align: center;  color: #bb1c25;"><span>was on <strong>{{strtoupper($data['updated_at']->format('jS'))}} DAY OF  {{strtoupper($data['updated_at']->format('M'))}}, {{$data['updated_at']->format('Y')}}</strong> duly registered for theimportation only of drugs, medicines and poisons to registereddistributors, wholesalers and hospitals in accordance with theprovisions of the Pharmacists Council of Nigeria Act, Cap PI7,LFN, 2004.</span></div>
            @endif
            @if($type == 'manufacturing_premises_renewal')
            <div style="margin-bottom: 20px; font-size: 25px; text-align: center;  color: #bb1c25;"><span>was on <strong>{{strtoupper($data['updated_at']->format('jS'))}} DAY OF  {{strtoupper($data['updated_at']->format('M'))}}, {{$data['updated_at']->format('Y')}}</strong> duly registered for themanufacturing and sales of such drugs, medicines and poisonsso manufactured to registered distributors, wholesalers andhospitals in accordance with the provisions of the PharmacistsCouncil of Nigeria Act, Cap P17, LFN, 2004</span></div>
            @endif
        </div>
