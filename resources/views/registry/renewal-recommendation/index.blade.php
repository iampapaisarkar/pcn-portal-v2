...
<div class="col-lg-12 col-md-12">
    <form id="approvedForm" class="w-100" method="POST" action="{{ route('registry-renewal-recommendation-approve-all') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <div class="row m-0">
                ...
                <div class="col-sm-12 col-md-6">
                    ...
                    <form method="GET" action="{{ route('registry-renewal-recommendation.index') }}">
                    @csrf
                        <label>Search:
            ...
            <table class="display table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($document->registration->type == 'hospital_pharmacy')
                            <a href="{{ route('registry-renewal-recommendation-show') }}?renewal_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->registration->type == 'ppmv')
                            <a href="{{ route('registry-renewal-recommendation-ppmv-show') }}?renewal_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->registration->type == 'community_pharmacy')
                            <a href="{{ route('registry-renewal-recommendation-community-show') }}?renewal_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->registration->type == 'distribution_premises')
                            <a href="{{ route('registry-renewal-recommendation-distribution-show') }}?renewal_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                            @if($document->registration->type == 'manufacturing_premises')
                            <a href="{{ route('registry-renewal-recommendation-manufacturing-show') }}?renewal_id={{$document->id}}&user_id={{$document->user->id}}">
                                <button class="btn btn-success btn-sm" type="button"><i class="nav-icon i-Pen-2"></i></button>
                            </a>
                            @endif
                        </td>
        ...
        <button onclick="approveSelected(event)" type="button" class="btn btn-primary mt-5">APPROVE SELECTED FOR FACILITY INSPECTION</button>
    </div>
</div>
</form>
</div>
    ...
    function approveSelected(event){
        ...
        $.confirm({
            title: 'APPROVE SELECTED FOR FACILITY INSPECTION',
            content: 'Are you sure want to approve for SELECTED registration?',
            buttons: {   
                ok: {
                    text: "YES",
